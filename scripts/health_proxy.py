...
def log(m):
    print(f"[tcp-proxy] {m}", flush=True)
...
def parse_http(data):
    try:
        ...
        first = head.split(b"\r\n", 1)[0].decode("ascii")
        p = first.split(" ", 2)
        if len(p) != 3 or not p[2].startswith("HTTP/"):
            return None
...
def handle_http(c, method, path):
    ...
    if path.startswith("/sub/"):
        t = token()
        if not t or path != "/sub/" + t or not SUB_FILE.is_file():
            c.sendall(response(404, "text/plain; charset=utf-8", "Not Found\n", head))
        else:
            c.sendall(response(200, "text/plain; charset=utf-8", SUB_FILE.read_bytes(), head))
        return True
    if path == "/sub":
        c.sendall(response(404, "text/plain; charset=utf-8", "Not Found\n", head))
        return True
    ...
    c.sendall(response(200, types.get(target.suffix.lower(), "application/octet-stream"), body, head))
    return True
