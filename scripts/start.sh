...
BUILD_ID="v8.9-dev-sni-matrix-20260813-02"
SNI_MATRIX_VERSION="3"
SUBSCRIPTION_GENERATOR_VERSION="4"
...
if [ -s "$UUID_FILE" ]; then UUID=$(tr -d '[:space:]' < "$UUID_FILE"); else UUID=$(xray uuid); [ -n "$UUID" ] || { echo "ERROR: xray uuid returned empty output" >&2; exit 1; }; printf '%s\n' "$UUID" > "$UUID_FILE"; fi
if [ -s "$PRIVATE_KEY_FILE" ] && [ -s "$PUBLIC_KEY_FILE" ]; then
  PRIVATE_KEY=$(tr -d '[:space:]' < "$PRIVATE_KEY_FILE"); PUBLIC_KEY=$(tr -d '[:space:]' < "$PUBLIC_KEY_FILE")
else
  ...
  printf '%s\n' "$PRIVATE_KEY" > "$PRIVATE_KEY_FILE"; printf '%s\n' "$PUBLIC_KEY" > "$PUBLIC_KEY_FILE"
fi
...
if [ -s "$VLESS_DECRYPTION_FILE" ] && [ -s "$VLESS_ENCRYPTION_FILE" ]; then
  VLESS_DECRYPTION=$(tr -d '[:space:]' < "$VLESS_DECRYPTION_FILE"); VLESS_ENCRYPTION=$(tr -d '[:space:]' < "$VLESS_ENCRYPTION_FILE")
else
  TMP="$DATA_DIR/.vlessenc-output.tmp"; umask 077
  ...
  rm -f "$TMP"; [ -n "$VLESS_DECRYPTION" ] && [ -n "$VLESS_ENCRYPTION" ] || { echo "ERROR: unable to parse ML-KEM-768 VLESS Encryption output" >&2; exit 1; }
  printf '%s\n' "$VLESS_DECRYPTION" > "$VLESS_DECRYPTION_FILE"; printf '%s\n' "$VLESS_ENCRYPTION" > "$VLESS_ENCRYPTION_FILE"; chmod 0600 "$VLESS_DECRYPTION_FILE" "$VLESS_ENCRYPTION_FILE"
fi

if [ -s "$SUB_TOKEN_FILE" ]; then SUBSCRIPTION_TOKEN=$(tr -d '[:space:]' < "$SUB_TOKEN_FILE"); else SUBSCRIPTION_TOKEN=$(python3 -c 'import secrets; print(secrets.token_urlsafe(32))'); printf '%s\n' "$SUBSCRIPTION_TOKEN" > "$SUB_TOKEN_FILE"; fi
...
printf '%s\n' "V89_BUILD_ID=$BUILD_ID" >&2
printf '%s\n' "V89_SNI_MATRIX_VERSION=$SNI_MATRIX_VERSION" >&2
printf '%s\n' "V89_SUBSCRIPTION_GENERATOR_VERSION=$SUBSCRIPTION_GENERATOR_VERSION" >&2
printf '%s\n' "V89_SOURCE_BRANCH=dev/v8.9-platform-hardening" >&2
printf '%s\n' "V89_SOURCE_MODE=direct-github-contents" >&2
printf '%s\n' "REALITY SNI server mode: $REALITY_SNI_SERVER_MODE" >&2

# Fail immediately if the image does not contain the expected development code.
[ -f /opt/xray/scripts/generate.py ] || { echo "ERROR: generate.py missing" >&2; exit 1; }
[ -f /opt/xray/scripts/generate_reality_sni_matrix.py ] || { echo "ERROR: SNI matrix generator missing; stale image detected" >&2; exit 1; }

python3 /opt/xray/scripts/health_proxy.py & HEALTH_PID=$!
cleanup(){ rm -f "$READY_FILE"; [ -n "$XRAY_PID" ] && kill "$XRAY_PID" 2>/dev/null || true; [ -n "$HEALTH_PID" ] && kill "$HEALTH_PID" 2>/dev/null || true; [ -n "$XRAY_PID" ] && wait "$XRAY_PID" 2>/dev/null || true; [ -n "$HEALTH_PID" ] && wait "$HEALTH_PID" 2>/dev/null || true; }
...
printf '%s\n' "V89_SUBSCRIPTION_TOTAL_NODES=$TOTAL_NODES" >&2
printf '%s\n' "V89_SUBSCRIPTION_REALITY_NODES=$REALITY_NODES" >&2
if [ "$REALITY_SNI_SERVER_MODE" = "multi" ]; then
  ...
  printf '%s\n' "V89_EXPECTED_REALITY_NODES=$EXPECTED_REALITY_NODES" >&2
...
if [ -n "$PUBLIC_DOMAIN" ]; then printf 'https://%s/sub/%s\n' "$PUBLIC_DOMAIN" "$SUBSCRIPTION_TOKEN" > "$DATA_DIR/subscription_url.txt"; chmod 0600 "$DATA_DIR/subscription_url.txt"; fi
